...
class Graph():
    def __init__(self, world_grid):
        ...
        '''
        nodes_tmp = [(j, i) for j in range(world_grid.shape[0])
                     for i in range(world_grid.shape[1])]
        ...
        for node, i in nodes_idx_dict.items():
            ...
            self.nodes[i] = {'grid_idx': node, 'edges': edges,
                             'edge_weights': weights, 'node_weight': np.inf}
        self.nodes[0]['node_weight'] = 0

    def select_min_unvisited_node(self, visisted_nodes):
        min_node, min_node_value = 0, np.inf
        for k, v in self.nodes.items():
            if v['node_weight'] == np.inf:
                continue
            if k in visisted_nodes:
                continue
    ...
    def find_path(self, print_out_logs=False):
        ...
        while True:
            ...
            for i, edge in enumerate(edges):
                old_node_weight = self.graph_world.nodes[edge[1]
                                                         ]['node_weight']
                new_node_weight = current_node_values['node_weight'] + \
                    current_node_values['edge_weights'][i]
                # Update connected nodes weights
                if old_node_weight == np.inf:
                    self.graph_world.nodes[edge[1]
                                           ]['node_weight'] = new_node_weight
            # Select the minimum node value
            new_node, new_node_value = self.graph_world.select_min_unvisited_node(
                visited_nodes)
            current_node = new_node
            visited_nodes.append(new_node)
            visited_node_grid_idxs.append(
                list(self.graph_world.nodes[new_node]['grid_idx']))
            world_vis[self.graph_world.nodes[current_node]['grid_idx'][0],
                      self.graph_world.nodes[current_node]['grid_idx'][1]] = new_node_value
            # Terminate as target node found
            if current_node == target_node:
                logs = self.output_logs(
                    world_vis[-1, -1], loop_steps, (time.perf_counter()-start_time)*1000, print_out_logs)
